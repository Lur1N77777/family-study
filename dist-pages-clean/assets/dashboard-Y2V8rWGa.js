import{a as u,s as n,i as o,r as l,b as k}from"./index-CJupM59u.js";import{a as $,s as S}from"./animations-BN6acAuJ.js";import{e as s}from"./escape-lW1fV86Q.js";async function E(a){a.innerHTML='<div style="padding:var(--space-8);text-align:center;color:var(--color-text-tertiary)">加载中...</div>',await u.refreshUser();const e=u.requireUser(),[r,i,d,p]=await Promise.all([n.getTasks(),n.getSubmissions(null,e.id),n.getProducts(),n.getFamilyUsers()]),c=r.map(t=>j(t)).sort((t,y)=>g(t)-g(y)),m=p.filter(t=>t.role==="parent"),v=p.filter(t=>t.role==="child"&&t.id!==e.id),b=e.points||0,f=i.filter(t=>t.status==="pending").length,h=i.filter(t=>t.status==="approved"&&new Date(t.created_at||t.createdAt).toDateString()===new Date().toDateString()).length,w=c.filter(t=>t.canSubmitNow).length,x=d.slice(0,3);a.innerHTML=`
